//-----------------------------------------------------------------------
// <copyright file="Keyboard.cs" company="YuGuan Corporation">
...
namespace DevLib.Input
{
    ...
    public class Keyboard
    {
        ...
        /// </summary>
        /// <remarks>
        /// Note that a combination of a combination of Key.Shift or Key.Capital and a Unicode point above 0xFE is not considered valid and will not result in the Unicode point being types without applying of the modifier key.
        /// </remarks>
        /// <param name="text">The text to type.</param>
        /// <param name="interval">The interval time, in milliseconds, between two keys.</param>
        public static void Type(string text, int interval = 100)
        {
            foreach (char character in text)
            {
                // If code point is bigger than 8 bits, for Unicode approach by setting wVk to 0.
                if (character > 0xFE)
                {
                    SendKeyboardKey(character, true, false, true, interval);
                    SendKeyboardKey(character, false, false, true, interval);
                }
                else
                {
                    int vKeyValue = NativeMethods.VkKeyScan(character);
                    ...
                    if (keyIsShifted)
                    {
                        Type(key, new Key[] { Key.Shift }, interval);
                    }
                    else
                    {
                        Type(key, interval);
        ...
        /// <param name="isKeyDown">Whether key is down.</param>
        /// <param name="isExtended">Whether key is extended.</param>
        /// <param name="isUnicode">Whether key is unicode.</param>
        /// <param name="interval">The interval time, in milliseconds, between two keys.</param>
        ...
        private static void SendKeyboardKey(ushort key, bool isKeyDown, bool isExtended, bool isUnicode, int interval)
        {
            ...
            NativeMethods.SendInput(1, new INPUT[] { input }, Marshal.SizeOf(input));

            Thread.Sleep(interval);
        }

        /// <summary>
        /// Get KeySpec from Key.
        /// </summary>
        /// <param name="key">The key to get.</param>
        /// <returns>KeySpec instance.</returns>
