...
namespace DevLib.Settings
{
    ...
    internal static class ExceptionHandler
    {
        ...
        public static void Log(Exception exception)
        {
            if (exception != null)
            {
                string message = string.Format("[{0}] [EXCEPTION] [{1}] [{2}]", DateTime.Now.ToString("yyyy-MM-ddTHH:mm:ss.fffUTCzzz"), GetStackFrameMethodInfo(new StackFrame(1)), exception.ToString());
